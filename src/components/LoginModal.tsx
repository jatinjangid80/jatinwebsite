...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  // Close modal automatically if user becomes signed in
  useEffect(() => {
    const handleAuth = () => {
      if (isOpen) {
        onClose();
  ...
  const loginAsDemoUser = (provider?: "google" | "github") => {
    setIsLoading(true);
    setOauthLoading(null);
    setErrorMessage("");
    const demoUser = {
      email: provider === "github" ? "jatin@github.com" : "jatin.jangid@gmail.com",
      user_metadata: {
  ...
  const handleOAuthLogin = async (provider: "google" | "github") => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        return; // Redirecting browser to OAuth provider
      } catch (err: any) {
        console.warn(`Supabase OAuth ${provider} error:`, err);
        setErrorMessage(
          err?.message
            ? `OAuth Error: ${err.message}`
            : `Could not connect with ${provider}. You can sign in using Demo mode below.`
        );
        setIsLoading(false);
        setOauthLoading(null);
        return;
    ...
    // Direct demo login if Supabase is not configured
    setTimeout(() => {
      loginAsDemoUser(provider);
    }, 500);
  };
            ...
            >
              ...
              <div className="login-social-group">
                <button
                  ...
                  disabled={isLoading}
                  className="login-social-btn login-btn-google"
                >
                  {oauthLoading === "google" ? (
                    <div className="login-spinner" style={{ width: 18, height: 18, borderWidth: 2 }} />
                  ) : (
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="login-social-btn login-btn-github"
                >
                  {oauthLoading === "github" ? (
                    <div className="login-spinner" style={{ width: 18, height: 18, borderWidth: 2 }} />
                  ) : (
              ...
              <AnimatePresence mode="wait">
                {errorMessage && (
                  <motion.div
                    ...
                    className="login-alert login-alert-error flex flex-col items-start gap-2"
                  >
                    ...
                    <button
                      ...
                      className="text-xs underline font-semibold hover:opacity-80 text-left cursor-pointer"
                    >
                      ⚡ Continue as Demo User (One-Click)
                    </button>
                  </motion.div>
                )}
